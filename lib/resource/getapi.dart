...
class getApi {
  ...
  Future<List<CovidModel>> fetchCovidList({String? query}) async {
    List<CovidModel> covidList = [];
    //var fullVideosUrl = ApiUrls().VIDEOS_URL;
    var response;
    try {
      response =
          await http.get(Uri.parse('https://owsnews.herokuapp.com/covid'));
    ...
    for (var jsonItem in jsonList) {
      ...
      if (query != null) {
        jsonList = jsonList
            .where((element) =>
                element.tinh!.toLowerCase().contains((query.toLowerCase())))
            .toList();
      }
